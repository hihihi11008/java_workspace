...
/*
 	...
 	자바에서는 모든 기본 자료형마다 1:1 대응하는 래퍼클래스는 지원해준다 
 	...
 	ex)숫자형문자를 실제 숫자로 변환 
 		기본자료형을 객체자료형으로 형변환 
 		기타 여러 메서드를 지원하므로써, 기본자료형을 보다 다양하게 제어할 수 있다
*/
...
public class WrapperApp {
	public static void main(String[] args) {
		...
		int z=Integer.parseInt(x); //static메서드이므로 Integer.찍고 접근한다 
		...
		int k = i; //원칙상 객체자료형을 기본자료형에 넣으려고 했으니, 불가능한 일이지만 위와 마찬가지로 래퍼클래스는 
						//기본자료형과 관련된 객체이므로, 내부적으로 unBoxing에 의해 기본자료형으로 변환이 된것이다 
