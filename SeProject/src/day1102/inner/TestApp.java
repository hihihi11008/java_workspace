...
public class TestApp extends JFrame{
	...
	public TestApp() {
		//켄버스를 현재 클래스에서만 사용하고, 추후 재사용성이 없을경우 굳이거푸집(.java)으로 정의할 이유가 있을까?
		//이럴땐, 즉 일회성의 객체인 경우 내부익명 클래스로 사용할 수 있다 
		//Inner Anonymous Class 내부익명 클래스라 한다 
		//형식은 앞의 new로 선언된 클래스를 구현한 클래스로 이해하면 된다 
		//주로 이벤트를 구현한 클래스는 해당 어플리케이션에서만 사용되므로, 
		//재사용가능성이 없다. 따라서 내부익명 클래스로 많이 사용된다 
		can = new Canvas() {
