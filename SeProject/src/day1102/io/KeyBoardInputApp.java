/*
 ...
 즉 실행중인 프로그램으로 데이터가 흘러드렁온다며, 이모든것이 다 입력 스트림이다. 
 따라서 이 예제에서는 파일을 대상으로 데이터를 읽는 것이 아니라 
 키보드를 대상으로 데이트를 읽어와 보자 
 주의) 파일과는 달리, 키보드의 스트림은 자바에서 생성할 수 있는 것이 아니라 ,이미 OS 차원에서 스트림이 
 		존재하므로, 자바는 단지 이미 존재하느 스트림을 얻어와 쓸 수 있을 뿐이다. 
 */
package day1102.io;
...
public class KeyBoardInputApp {
	public static void main(String[] args) {
		...
		InputStream is =System.in; // 표준 입력스트림(키보드 or 기타 입력도구)
		InputStreamReader reader = new InputStreamReader(is); //표준 입력스트림(키보드 or 기타 입력도구)
		int data;
		try { //shift+Alt+z 
			while (true) {
				data = reader.read();//1byte 읽기  , read()의 특징, 입력이 완료되지 않으면 무함대기상태로 머물러있음 
