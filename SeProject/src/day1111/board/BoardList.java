...
public class BoardList extends JPanel{
	...
	//게시물 가져오기 
	//rs에 담겨진 데이터를 TableModel이 보유한 이차원 배열 data 에 대입 
	public void getList() {
	      ...
	      try {
			...
			//스크롤이 가능하고 (next()능력+ 자유자재로 이동능력), 읽기전용의 rs 를 만들기 위한 옵션
			rs=pstmt.executeQuery();
			
			rs.last();
			int total = rs.getRow();//총 레코드수 반환 
			...
			while (rs.next()) {
				...
				records[index++]=arr;//일차원배열을 이차원배열의 방에 담음
			}	
			//이차원배열이 모두 완성되었으므로, TableModel이 보유한 data 이차원 배열에 대입! 
			boardModel.data=records;
			table.updateUI();//JTable UI갱신
